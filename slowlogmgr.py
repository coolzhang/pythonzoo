...
# slowlogmgr --server=root:12345@localhost:3308 --rotate
"""
VERSION = "%prog Alpha"
...
optparser.add_option("--rotate", action="store_true", dest="rotate", default=False,
                     help="Rotate slow log")
...
def enable_slowlog():
    try:
        ...
        query1 = ("set global slow_query_log = 0")
        query1_1= ("alter table mysql.slow_log engine=myisam")
        query1_2= ("alter table mysql.slow_log add index idx_stime(start_time)")
        query2 = ("set global log_output = 'table'")
        query3 = ("set global slow_query_log = 1")
        query4 = ("set global long_query_time = %s")
        cur_enable_slowlog.execute(query1)
        cur_enable_slowlog.execute(query1_1)
        cur_enable_slowlog.execute(query1_2)
        cur_enable_slowlog.execute(query2)
        cur_enable_slowlog.execute(query3)
        querytime = float(options.querytime) if '.' in 'options.querytime' else int(options.querytime)
        cur_enable_slowlog.execute(query4, (querytime,))
        cnx.close()
    except mysql.connector.errors.Error as err:
        print "Database connection or operation went wrong: %s" %(err)
    except (KeyboardInterrupt, SystemExit):
        ...
        if options.server:
            if options.runtime and not options.daemon and not options.account:
                ...
                try:
                    ...
                    print
                    print "slowlogmgr.py is stopped."
            elif (not options.runtime and not options.account and not options.rotate) or \
                 (not options.daemon and not options.account and not options.rotate):
